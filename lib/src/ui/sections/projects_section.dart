  ...
  List<Project> _getFilteredProjects() {
    ...
    switch (_selectedFilter) {
      case ProjectFilter.all:
        return allProjects;
      case ProjectFilter.mobile:
        // لێرە مە کۆد گوهۆڕی: هەر پڕۆژەیەک یێ 'Portfolio' نەبیت، دێ وەک Mobile دیار بیت
        return allProjects.where((p) => !p.title.contains('Portfolio')).toList();
      case ProjectFilter.web:
        // لێرە بتنێ پڕۆژێ پۆرتفۆلیۆیێ دێ دیار بیت
        return allProjects.where((p) => p.title.contains('Portfolio')).toList();
    }
  }
                ...
                child: Wrap(
                  ...
                  children: ProjectFilter.values.map((filter) {
                    ...
                    return FilterChip(
                      ...
                      side: BorderSide(
                        color: isSelected ? theme.colorScheme.primary : (isDark ? const Color(0xFF233554) : Colors.grey[300]!),
                        width: 1.5,
                      ),
